...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (!args[0]) {
      return message.channel.send("`Gunakan: .createrole <colorname> <Name>`");
    }
    if (!name) {
      return message.channel.send("You need to specify a name for your Role!!! Gunakan: .createrole <colorname> <Name>");
    }
    if (regex === false) {
