...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (!member) return message.channel.send("kau perlu mention seseorang yang kamu peluk");
    let HugEmbed = new Discord.MessageEmbed()
      .setTitle(
        `${message.author.username} kau ga bisa peluk disi sendiri, mau ku peluk??`
      )
    ...
    let HugEmbed2 = new Discord.MessageEmbed()
      .setTitle(
        `${message.author.username} peluk erat ${member.user.username}, manisnya pelukanku..`
      )
      .setImage(String([images[image]]))
      .setColor(0xf000ff);
