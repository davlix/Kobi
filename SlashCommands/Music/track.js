...
module.exports = {
    ...
    try {
      ...
      .setAuthor("Kobi musik", client.user.displayAvatarURL())
      .setDescription(`👍 Masuk ${queue.connection.channel.toString()} dan 📄 cmd ke ${queue.metadata.channel.toString()}`)
      ...
      return client.say.errorMessage(interaction, `Gabisa masuk voice channel mu!\n\`${error}\``);
    }
    ...
    if (!queue || !queue.playing)
      return client.say.errorMessage(interaction, "Saat ini saya tidak bermain di guild ini.");

    if (!client.utils.canModifyQueue(interaction)) return;

    if (queue.connection.paused)
      return client.say.warnMessage(interaction, "Lagu sudah di pause.");

    queue.setPaused(true);
    return client.say.infoMessage(interaction, "Paused lagu saat ini.");
